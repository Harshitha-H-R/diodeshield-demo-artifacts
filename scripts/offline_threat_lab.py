...
def run(scenario: str, count: int, batch_size: int) -> dict[str, object]:
    ...
    for offset in range(0, count, batch_size):
        ...
        for index in range(offset, min(offset + batch_size, count)):
            ...
            destination = (
                f"10.0.1.{(index % 32) + 1}" if scenario == "behavior" else "10.0.0.8"
            )
            batch.append(
                TrafficEvent(
                    ...
                    metadata={"payload_altered": scenario == "altered",
                              "virtual_identity": scenario == "spoof"},
                )
            )
        captured += len(batch)
...
def main() -> None:
    ...
    parser.add_argument("--scenario", choices=["flood", "spoof", "ttl", "altered", "behavior"], default="flood")
